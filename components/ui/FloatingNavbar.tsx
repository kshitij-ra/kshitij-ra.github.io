...
export const FloatingNav = ({
  ...
  useMotionValueEvent(scrollYProgress, "change", (current) => {
    if (typeof current === "number" && isScrollable) {
      ...
      // If not scrollable, always keep the navbar visible
      setVisible(true);
    }
  });
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "flex max-w-fit md:min-w-[70vw] lg:min-w-fit fixed z-[5000] top-8 inset-x-0 mx-auto px-10 py-5 rounded-lg border border-black/.1 shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] items-center justify-center space-x-4",
          className
        )}
        style={{
